...
describe('findJsFiles', () => {
    ...
    it('should return an array of file objects', () => {
        jest
            ...
            { name: 'test3.csv', isDirectory: () => false },
            {
            ...
            .mockReturnValueOnce([
            { name: 'test4.js', isDirectory: () => false },
        ]);
        ...
        // eslint-disable-next-line  @typescript-eslint/no-unsafe-member-access
        expect(jsFiles[2].filePath).toBe(path.join(targetPath, 'testSubDirectory', 'test4.js'));
        // eslint-disable-next-line  @typescript-eslint/no-unsafe-member-access
...
describe('createRegexpFromFileNames', () => {
    it('should return an array of RegExp objects', () => {
        ...
        expect(regexpArr[0]).toEqual(new RegExp(postbuild.TARGET_REGEXP_STR.replace('{{fileName}}', 'test1'), 'gm'));
        expect(regexpArr[1]).toBeInstanceOf(RegExp);
        expect(regexpArr[1]).toEqual(new RegExp(postbuild.TARGET_REGEXP_STR.replace('{{fileName}}', 'test2'), 'gm'));
    });
...
describe('replaceFileContent', () => {
    it('should replace the content of a file', () => {
        const fileContent = "import { test1 } from './test1';\nimport test2, { test2sub } from './test2';\nimport fs from 'fs';\nimport test2 from 'test2';\nimport * as utils from './test2';\nconst test2 = () => {\n  console.log('test2');\n};\n\nexport { test2 };\n";
        const regexp = new RegExp(postbuild.TARGET_REGEXP_STR.replace('{{fileName}}', 'test2'), 'gm');
        const extension = '.js';
        expect(fileContent.match(regexp)).not.toBeNull();
        expect(postbuild.replaceFileContent(fileContent, regexp, extension)).toBe("import { test1 } from './test1';\nimport test2, { test2sub } from './test2.js';\nimport fs from 'fs';\nimport test2 from 'test2';\nimport * as utils from './test2.js';\nconst test2 = () => {\n  console.log('test2');\n};\n\nexport { test2 };\n");
    });
});
describe('postbuild', () => {
    ...
    const expectedFileContent = "import { authorize, isAuthorized } from '../test.js';";
    beforeEach(() => {
...
//# sourceMappingURL=postbuild.test.js.map