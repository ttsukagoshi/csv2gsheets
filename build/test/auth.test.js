...
describe('isAuthorized', () => {
    ...
    it('should throw an error if credential file does not exist', () => {
        ...
        const result = () => auth.isAuthorized();
        // Assert
        expect(result).toThrow(new C2gError(MESSAGES.error.c2gErrorCredentialsFileNotFound));
...
describe('saveToken', () => {
    ...
    it('should throw an error if credentials file does not have top level key of `installed` or `web`', () => {
        ...
        // Act & Assert
        expect(() => auth.saveToken(mockClient)).toThrow(new C2gError(MESSAGES.error.c2gErrorInvalidCredentials));
    });
...
describe('authorize', () => {
    ...
    it('should return OAuth2Client if token file exists', async () => {
        ...
        jest
            ...
            .mockImplementation(() => mockClient); // loadSavedToken
        // Act
    ...
    it('should create and return a new OAuth2Client after running `authenticate` and `saveToken`', async () => {
        ...
        expect(fs.writeFileSync).toHaveBeenCalledTimes(1);
        expect(fs.writeFileSync).toHaveBeenCalledWith(auth.TOKEN_PATH, JSON.stringify(mockToken));
    });
});
