...
describe('convert', () => {
    ...
    it('should throw an error if the user is not logged in', async () => {
        ...
        await expect(convert({})).rejects.toThrow(new C2gError(MESSAGES.error.c2gErrorNotLoggedIn));
    });
    it('should throw an error if there are no CSV files in the designated local directory', async () => {
        ...
        await expect(convert({})).rejects.toThrow(new C2gError(MESSAGES.error.c2gErrorNoCsvFilesFound));
    });
    it('should show the complete conversion process on dry run', async () => {
        // Arrange
        const mockLocalCsvFiles = ['file1.csv', 'file2.CSV'];
        const mockLocalCsvFilePaths = mockLocalCsvFiles.map((file) => path.join(mockConfig.sourceDir, file));
        jest.spyOn(auth, 'isAuthorized').mockImplementation(() => true);
        jest.spyOn(console, 'info').mockImplementation();
        jest
            .spyOn(utils, 'readConfigFileSync')
        ...
        // Assert
        expect(console.info).toHaveBeenNthCalledWith(1, `${MESSAGES.log.runningOnDryRun}\n\n${MESSAGES.log.convertingCsvWithFollowingSettings(Object.keys(mockConfig)
            .map((key) => `${key}: ${mockConfig[key]}`)
            .join('\n  '))}`);
        expect(console.info).toHaveBeenNthCalledWith(2, MESSAGES.log.processingCsvFile(mockLocalCsvFiles[0], null));
        expect(console.info).toHaveBeenNthCalledWith(3, MESSAGES.log.processingCsvFileComplete);
        expect(console.info).toHaveBeenNthCalledWith(4, MESSAGES.log.processingCsvFile(mockLocalCsvFiles[1], null));
        expect(console.info).toHaveBeenNthCalledWith(5, MESSAGES.log.processingCsvFileComplete);
    });
    it('should open the default web browser if --browse option is enabled', async () => {
        ...
        // Assert
        expect(mockOpen).toHaveBeenCalledWith(`https://drive.google.com/drive/folders/${mockConfig.targetDriveFolderId}`);
        expect(console.info).toHaveBeenNthCalledWith(6, MESSAGES.log.openingTargetDriveFolderOnBrowser(`https://drive.google.com/drive/folders/${mockConfig.targetDriveFolderId}`));
    });
    ...
    it('should complete the conversion process on dry run with updateExistingGoogleSheets being true', async () => {
        // Arrange
        const mockConfigWithUpdateExistingGoogleSheets = {
            ...mockConfig,
        ...
        const mockLocalCsvFiles = ['file1.csv', 'file2.CSV'];
        const mockLocalCsvFilePaths = mockLocalCsvFiles.map((file) => path.join(mockConfigWithUpdateExistingGoogleSheets.sourceDir, file));
        jest.spyOn(auth, 'isAuthorized').mockImplementation(() => true);
        jest.spyOn(console, 'info').mockImplementation();
        jest
            .spyOn(utils, 'readConfigFileSync')
    ...
    it('should complete the conversion process with updateExistingGoogleSheets=true', async () => {
        ...
        expect(mockDrive.files.update).toHaveBeenCalledWith({
            supportsAllDrives: mockConfigWithUpdateExistingGoogleSheets.targetIsSharedDrive,
    ...
    it('should complete the conversion process with updateExistingGoogleSheets=true, targetDriveFolderId=root, --browse=true', async () => {
        // Arrange
        const mockConfigWithTargetRoot = {
            ...mockConfig,
            updateExistingGoogleSheets: true,
        ...
        // except for mockConfig being replaced with mockConfigWithUpdateExistingGoogleSheets
        const mockLocalCsvFiles = ['file1.csv', 'file2.CSV'];
        const mockLocalCsvFilePaths = mockLocalCsvFiles.map((file) => path.join(mockConfigWithTargetRoot.sourceDir, file));
        jest.spyOn(auth, 'isAuthorized').mockImplementation(() => true);
        jest.spyOn(console, 'info').mockImplementation();
        jest
            .spyOn(utils, 'readConfigFileSync')
        ...
        // Assert
        expect(mockOpen).toHaveBeenCalledWith('https://drive.google.com/drive/my-drive');
        expect(console.info).toHaveBeenNthCalledWith(4, MESSAGES.log.processingCsvFile(mockLocalCsvFiles[1], null));
        expect(console.info).toHaveBeenNthCalledWith(6, MESSAGES.log.openingTargetDriveFolderOnBrowser('https://drive.google.com/drive/my-drive'));
    });
    it('should complete the conversion process with saveOriginalFilesToDrive=true', async () => {
        // Arrange
        const mockConfigWithSaveOriginalFilesToDrive = {
            ...mockConfig,
        ...
        // except for mockConfig being replaced with mockConfigWithUpdateExistingGoogleSheets
        const mockLocalCsvFiles = ['file1.csv', 'file2.CSV'];
        const mockLocalCsvFilePaths = mockLocalCsvFiles.map((file) => path.join(mockConfigWithSaveOriginalFilesToDrive.sourceDir, file));
        jest.spyOn(auth, 'isAuthorized').mockImplementation(() => true);
        jest.spyOn(console, 'info').mockImplementation();
        jest
            .spyOn(utils, 'readConfigFileSync')
        ...
        await convert({});
        // Assert
        expect(console.info).toHaveBeenNthCalledWith(2, MESSAGES.log.uploadingOriginalCsvFilesTo(mockCsvFolderId));
